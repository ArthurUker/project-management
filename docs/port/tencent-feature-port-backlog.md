...
## N. 范围变更：旧数据不迁移（2026-09-10 用户裁定）

**结论**：Tencent 旧系统部署在 Windows 服务器上，其设计与本系统（PostgreSQL-only + 结构化模型 + RBAC）不兼容 →
原规划中「**数据迁移 ETL 独立主任务**」整块取消。

**随之取消**（原规划第 1–9 步全部作废）：
1. 冻结旧系统写入、旧库与附件备份；
2. 查清 Tencent 生产真实库类型；
3. 旧字段 → 新字段、中文状态 → 枚举值映射表；
4. 保留原 ID、按外键顺序导入用户/角色/项目/阶段/任务/模板/汇报/知识库/试剂/附件；
5. 模板 JSON 拆分为结构化表；
6. 密码/名称/角色/审阅字段迁移；
7. `CodeSequence` 重算（**改为：新系统自增发号，正常运行即可，无需对齐旧编号**）；
8. 迁移前后数量/关联/孤儿/附件校验值比对；
9. staging 全量迁移演练与失败回滚演练。

**不取消（仍然有效）**：
- 本系统自身的备份/恢复（批次三已交付）——用途转为**日常 JSON 备份与回灌**，与旧系统无关；
- `CodeSequence`、枚举字典、软删除等运行时机制照常工作；
- 生产库类型之争（README 与 schema 不一致）**不再是迁移阻塞项**，仅为运维文档准确性问题。
